...
class Header(BaseComponent):
    ...
    def build_component(self, entity_id, model):
        ...
        return H1(model.name.upper())  # noqa: F405
...
class LineChart(MatplotlibViz):
    ...
    def visualization(self, asset_id, model):
        ...
        # data for the asset
        e_count = model.event_counts(asset_id)

        # Use the pandas .fillna method to fill nulls with 0
        e_count = e_count.fillna(0)

        # User the pandas .set_index method to set
        # the date column as the index
        e_count = e_count.set_index('event_date')

        # Sort the index
        e_count = e_count.sort_index()

        # Use the .cumsum method to change the data
        # in the dataframe to cumulative counts
        e_count = e_count.cumsum()

        # Set the dataframe columns to the list
        # ['Positive', 'Negative']
        e_count.columns = ['Positive', 'Negative']
...
async def update_data(r):
    ...
    model_id = data._dict['user-selection']
    if profile_type == 'Employee':
        return RedirectResponse(f"/employee/{model_id}", status_code=303)
    elif profile_type == 'Team':
        return RedirectResponse(f"/team/{model_id}", status_code=303)
